...
(function () {
  ...
  const THREE_CAMERA_MODELS = Object.freeze(["absolute_bounds"]);
  ...
  const THREE_MATERIAL_DEBUG_LIMITS = Object.freeze({
    ...
    meteorTextureEmissiveIntensity: { min: 0.8, max: 1.6 },
  });
  ...
  const threeState = {
    ...
    cosmicDustMeshes: new Map(),
    cosmicDustMaterial: null,
    harmonicDustMaterials: new Map(),
    harmonicDustGeometry: null, cosmicDustGeometry: null,
    planetGeometry: null,
  ...
  function resetMeteorTextureFrameDiagnostics() {
    ...
    diagnostics.meteorTextureAssignmentsByColor = {};
    diagnostics.meteorTextureMapReadyByColor = {};
    diagnostics.meteorTextureEmissiveReadyByColor = {};
    diagnostics.meteorTexturePendingApplyCount = 0;
    diagnostics.meteorTextureInstancesWithExternalMap = 0;
    diagnostics.meteorTextureInstancesWithExternalEmissiveMap = 0;
    diagnostics.meteorTextureInstancesSkippedBecauseGlbHadMap = 0;
    diagnostics.meteorTextureInstancesSkippedBecauseGlbHadEmissiveMap = 0;
  ...
  function resetDiagnostics() {
    ...
    threeState.meteorTextureDiagnostics = createMeteorTextureDiagnostics();
  }
  ...
  function getThreeMaterialSettings() {
    ...
    return {
      ...
      meteorPngTexturesEnabled: merged.meteorPngTexturesEnabled !== false,
      redMeteorTexturesEnabled: merged.redMeteorTexturesEnabled !== false && merged.meteorPngTexturesEnabled !== false,
      yellowMeteorTexturesEnabled: merged.yellowMeteorTexturesEnabled !== false && merged.meteorPngTexturesEnabled !== false,
      meteorTextureEmissiveIntensity: clampNumber(merged.meteorTextureEmissiveIntensity, THREE_MATERIAL_DEBUG_DEFAULTS.meteorTextureEmissiveIntensity, THREE_MATERIAL_DEBUG_LIMITS.meteorTextureEmissiveIntensity.min, THREE_MATERIAL_DEBUG_LIMITS.meteorTextureEmissiveIntensity.max),
    };
  }
  ...
  function setThreeMaterialDebugSetting(key, value) {
    ...
    if (key === "enabled") next.enabled = value === true || value === "true" || value === "1";
      ...
      next.yellowMeteorTexturesEnabled = next.meteorPngTexturesEnabled;
    }
    else if (key === "redMeteorTexturesEnabled") {
      next.redMeteorTexturesEnabled = value === true || value === "true" || value === "1";
      next.meteorPngTexturesEnabled = next.redMeteorTexturesEnabled || next.yellowMeteorTexturesEnabled;
    }
    else if (key === "yellowMeteorTexturesEnabled") {
      next.yellowMeteorTexturesEnabled = value === true || value === "true" || value === "1";
      next.meteorPngTexturesEnabled = next.redMeteorTexturesEnabled || next.yellowMeteorTexturesEnabled;
    }
    else if (key === "meteorTextureEmissiveIntensity") next.meteorTextureEmissiveIntensity = clampNumber(value, current.meteorTextureEmissiveIntensity, THREE_MATERIAL_DEBUG_LIMITS.meteorTextureEmissiveIntensity.min, THREE_MATERIAL_DEBUG_LIMITS.meteorTextureEmissiveIntensity.max);
    window.HC = window.HC || {};
    window.HC.WorldRendererDebug = window.HC.WorldRendererDebug || {};
  ...
  function emitMeteorTextureDebugEvent(type, payload = {}) {
    return emitGlbDebugEvent(type, Object.assign({ assetKind: "meteor_texture" }, payload));
  }
  ...
  function loadMeteorTexture(THREE, assignment) {
    const logicalPath = assignment?.path || null;
    if (!logicalPath || !THREE?.TextureLoader) return null;
    const resolvedUrl = resolvePublicAssetPath(logicalPath);
    let entry = threeState.meteorTextureCache.get(logicalPath);
    ...
    entry = {
      ...
      rawUrl: logicalPath,
      path: logicalPath,
      name: assignment.path?.split("/").pop() || resolvedUrl,
    ...
    threeState.meteorTextureCache.set(logicalPath, entry);
    threeState.meteorTextureDiagnostics.textureLoadRequests += 1;
    emitMeteorTextureDebugEvent("world.texture_load_started", { logicalPath, resolvedUrl, url: resolvedUrl, colorKey: entry.colorKey, kind: entry.kind, status: "loading" });
    entry.promise = threeState.textureLoader.loadAsync(resolvedUrl)
      .then((loadedTexture) => {
        entry.texture = configureMeteorTexture(THREE, loadedTexture, assignment.kind);
        const image = entry.texture?.image || null;
        ...
        emitMeteorTextureDebugEvent("world.texture_load_ready", { logicalPath, resolvedUrl, url: resolvedUrl, colorKey: entry.colorKey, kind: entry.kind, status: "ready", durationMs: entry.durationMs, width: entry.imageWidth, height: entry.imageHeight });
      ...
      .catch((error) => {
        entry.durationMs = performance.now() - startedAt;
        entry.status = "failed";
        entry.errorMessage = error?.message || String(error || "load error");
        entry.error = entry.errorMessage;
        threeState.meteorTextureDiagnostics.lastFallbackReason = `texture_failed:${entry.name}`;
        threeState.meteorTextureDiagnostics.meteorTextureLastError = `${entry.name}: ${entry.errorMessage}`;
        markMeteorTextureMaterialsForUpdate(entry);
        warnMeteorTextureOnce(resolvedUrl, entry.errorMessage);
        emitMeteorTextureDebugEvent("world.texture_load_failed", { logicalPath, resolvedUrl, url: resolvedUrl, colorKey: entry.colorKey, kind: entry.kind, status: "failed", durationMs: entry.durationMs, errorMessage: entry.errorMessage });
        return null;
      });
    return entry;
  }

  function preloadMeteorTexturePalettes(THREE) {
    ...
    for (const colorKey of Object.keys(METEOR_TEXTURE_PALETTES)) {
      if (!isMeteorTextureColorEnabled(colorKey, settings)) continue;
      for (const kind of ["map", "emissiveMap"]) {
        buildMeteorTexturePalette(colorKey, kind).forEach((assignment) => loadMeteorTexture(THREE, assignment));
      }
    }
  }
  ...
  function materialHasUsableTexture(texture) {
    return Boolean(texture && texture.image);
  }
  ...
  function getMeteorTextureReadinessByColor(kind) {
    const counts = {};
    for (const colorKey of Object.keys(METEOR_TEXTURE_PALETTES)) {
      counts[colorKey] = 0;
      for (const paletteEntry of buildMeteorTexturePalette(colorKey, kind)) {
        const cacheEntry = threeState.meteorTextureCache.get(paletteEntry.path);
        if (cacheEntry?.status === "ready" && materialHasUsableTexture(cacheEntry.texture)) counts[colorKey] += 1;
      }
    }
    return counts;
  }

  function countMeteorTextureAssignmentsByColor() {
    const counts = {};
    for (const colorKey of Object.keys(METEOR_TEXTURE_PALETTES)) counts[colorKey] = 0;
    for (const entry of threeState.meteorMeshes.values()) {
      const colorKey = entry?.textureAssignment?.colorKey || entry?.meteorTextureAssignments?.map?.colorKey || null;
      if (counts[colorKey] != null) counts[colorKey] += 1;
    }
    return counts;
  }
  ...
  function getMeteorTextureEvidence() {
    ...
    const evidence = Object.assign(createMeteorTextureDiagnostics(), diagnostics, {
      ...
      meteorTextureLastError: diagnostics.meteorTextureLastError || null,
    });
    for (const entry of threeState.meteorMeshes.values()) {
    ...
    evidence.meteorTextureAssignmentsByColor = countMeteorTextureAssignmentsByColor();
    evidence.meteorTextureMapReadyByColor = getMeteorTextureReadinessByColor("map");
    evidence.meteorTextureEmissiveReadyByColor = getMeteorTextureReadinessByColor("emissiveMap");
    evidence.meteorTextureLastError = diagnostics.meteorTextureLastError || null;
  ...
  function applyMeteorTexturesToMaterial(THREE, material, colorKey, mapTextureEntry, emissiveTextureEntry) {
    if (!material) return { material, changed: false, hasMap: false, hasEmissiveMap: false, skippedImportedMap: false, skippedImportedEmissiveMap: false, appliedMap: false, appliedEmissiveMap: false };
    const config = getMeteorTextureConfig(colorKey);
    ...
    if (!skipEmissive && emissiveTexture && targetMaterial.emissiveMap !== emissiveTexture) {
      ...
      changed = true;
      appliedEmissiveMap = true;
    }
    if (targetMaterial.color && config.baseColor != null && targetMaterial.color.getHex?.() !== config.baseColor && (appliedMap || appliedEmissiveMap)) {
    ...
    if ((appliedEmissiveMap || targetMaterial.userData?.hcMeteorTextureAppliedEmissiveUrl) && targetMaterial.emissive && config.emissiveColor != null && targetMaterial.emissive.getHex?.() !== config.emissiveColor) {
      targetMaterial.emissive.setHex(config.emissiveColor);
      changed = true;
    }
    const settings = threeState.materialSettings || getThreeMaterialSettings();
    const emissiveIntensity = Number.isFinite(settings.meteorTextureEmissiveIntensity) ? settings.meteorTextureEmissiveIntensity : config.emissiveIntensity;
    if ((appliedEmissiveMap || targetMaterial.userData?.hcMeteorTextureAppliedEmissiveUrl) && "emissiveIntensity" in targetMaterial && targetMaterial.emissiveIntensity !== emissiveIntensity) {
      targetMaterial.emissiveIntensity = emissiveIntensity;
      changed = true;
    }
    if ((appliedMap || appliedEmissiveMap) && "roughness" in targetMaterial && Number.isFinite(config.roughness) && targetMaterial.roughness !== config.roughness) { targetMaterial.roughness = config.roughness; changed = true; }
    if ((appliedMap || appliedEmissiveMap) && "metalness" in targetMaterial && Number.isFinite(config.metalness) && targetMaterial.metalness !== config.metalness) { targetMaterial.metalness = config.metalness; changed = true; }
    if (changed) targetMaterial.needsUpdate = true;
    return {
      material: targetMaterial,
      changed,
      hasMap: !!targetMaterial.map,
      hasEmissiveMap: !!targetMaterial.emissiveMap,
      skippedImportedMap: skipMap,
      skippedImportedEmissiveMap: skipEmissive,
      appliedMap,
      appliedEmissiveMap,
    };
  }
  ...
  function getMeteorTexturePassEnabled(settings) {
    return settings.meteorPngTexturesEnabled !== false
      && (settings.redMeteorTexturesEnabled !== false || settings.yellowMeteorTexturesEnabled !== false)
      && (settings.materialMode || "imported") === "imported";
  }
  ...
  function buildMeteorTextureApplySignature(entry, colorKey, settings) {
    return JSON.stringify({
      ...
      redMeteorTexturesEnabled: settings?.redMeteorTexturesEnabled !== false,
      yellowMeteorTexturesEnabled: settings?.yellowMeteorTexturesEnabled !== false,
    });
  }
  ...
  function getMeteorTextureMaterialCoverage(entry, expectedMapUrl, expectedEmissiveUrl) {
    ...
    entry?.glb?.traverse?.((object) => {
      ...
      materials.forEach((material) => {
        ...
        if (expectedMapUrl && !materialHasUsableTexture(material?.map)) coverage.missingExpectedMap += 1;
        if (expectedMapUrl && materialHasUsableTexture(material?.map) && appliedMapUrl && appliedMapUrl !== expectedMapUrl) coverage.missingExpectedMap += 1;
        if (expectedEmissiveUrl && !materialHasUsableTexture(material?.emissiveMap)) coverage.missingExpectedEmissiveMap += 1;
        if (expectedEmissiveUrl && materialHasUsableTexture(material?.emissiveMap) && appliedEmissiveUrl && appliedEmissiveUrl !== expectedEmissiveUrl) coverage.missingExpectedEmissiveMap += 1;
      });
    });
    return coverage;
  }
  ...
  function meteorTextureEntryAlreadyCurrent(entry, signature) {
    ...
    return (!expectedMapUrl || coverage.missingExpectedMap <= 0) && (!expectedEmissiveUrl || coverage.missingExpectedEmissiveMap <= 0);
  }
  ...
  function syncMeteorTexturePaletteForEntry(THREE, entry) {
    ...
    const config = getMeteorTextureConfig(colorKey);
    const colorEnabled = isMeteorTextureColorEnabled(colorKey, settings);
    const hasColor = !!colorKey && colorKey !== "neutral";
    const skipReason = config ? (colorEnabled ? null : "disabled_color") : (hasColor ? "unsupported_color" : "missing_color_key");
    ...
    if (!config || !enabled || !colorEnabled || !entry.meteorTextureAssignments?.map || !entry.meteorTextureAssignments?.emissiveMap) {
      ...
      entry.meteorTextureLastToggleEnabled = enabled;
      entry.meteorTextureStatus = config ? (colorEnabled ? "disabled" : "disabled_color") : skipReason;
      diagnostics.lastFallbackReason = !config ? skipReason : (colorEnabled ? "disabled" : "disabled_color");
      return;
    }
    ...
    if (!mapEntry && !emissiveEntry) {
      diagnostics.lastFallbackReason = "texture_unavailable";
      return;
    }
    const currentMapUrl = mapEntry?.status === "ready" && materialHasUsableTexture(mapEntry?.texture) ? mapEntry.url : null;
    const currentEmissiveUrl = emissiveEntry?.status === "ready" && materialHasUsableTexture(emissiveEntry?.texture) ? emissiveEntry.url : null;
    if (!currentMapUrl || !currentEmissiveUrl) diagnostics.meteorTexturePendingApplyCount += 1;
    let materialsWithMap = 0;
    let materialsWithEmissiveMap = 0;
    let mapAppliedToSlot = 0;
    let emissiveAppliedToSlot = 0;
    entry.glb.traverse?.((object) => {
      if (!object.isMesh) return;
      ...
      const nextMaterials = materials.map((material) => {
        ...
        if (result.skippedImportedMap) {
          diagnostics.meteorTextureInstancesSkippedBecauseGlbHadMap += 1;
          material.userData = material.userData || {};
          if (!material.userData.hcMeteorSkippedImportedMapEvent) {
            material.userData.hcMeteorSkippedImportedMapEvent = true;
            emitMeteorTextureDebugEvent("world.meteor_texture_skipped_imported_map", { visualId: entry.visualId, colorKey, url: currentMapUrl, materialName: material?.name || null });
          }
        }
        if (result.skippedImportedEmissiveMap) {
          diagnostics.meteorTextureInstancesSkippedBecauseGlbHadEmissiveMap += 1;
          material.userData = material.userData || {};
          if (!material.userData.hcMeteorSkippedImportedEmissiveEvent) {
            material.userData.hcMeteorSkippedImportedEmissiveEvent = true;
            emitMeteorTextureDebugEvent("world.meteor_texture_skipped_imported_emissive", { visualId: entry.visualId, colorKey, url: currentEmissiveUrl, materialName: material?.name || null });
          }
        }
        if (result.appliedMap) mapAppliedToSlot += 1;
        if (result.appliedEmissiveMap) emissiveAppliedToSlot += 1;
    ...
    const allTextureRequestsSettled = (!mapEntry || mapEntry.status !== "loading") && (!emissiveEntry || emissiveEntry.status !== "loading");
    entry.meteorTextureApplySignature = (currentMapUrl && currentEmissiveUrl) || allTextureRequestsSettled ? signature : null;
    entry.meteorTextureLastMaterialMode = settings.materialMode || "imported";
    entry.meteorTextureLastToggleEnabled = enabled;
    ...
    if (mapAppliedToSlot > 0) { diagnostics.mapApplied += 1; diagnostics.meteorTextureInstancesWithExternalMap += 1; }
    if (emissiveAppliedToSlot > 0) { diagnostics.emissiveMapApplied += 1; diagnostics.meteorTextureInstancesWithExternalEmissiveMap += 1; }
    ...
    if (mapAppliedToSlot > 0 || emissiveAppliedToSlot > 0) {
      emitMeteorTextureDebugEvent("world.meteor_texture_applied", { visualId: entry.visualId, colorKey, mapUrl: currentMapUrl, emissiveMapUrl: currentEmissiveUrl, mapSlots: mapAppliedToSlot, emissiveSlots: emissiveAppliedToSlot });
    }
  }
  ...
  function assignMeteorGlbAsset(entry, colorKey, { countReassignment = false } = {}) {
    ...
    if (getMeteorTextureConfig(colorKey)) {
      ...
      entry.textureAssignment = {
        ...
        emissiveMapUrl: entry.meteorTextureAssignments.emissiveMap?.url || null,
      };
      entry.root.userData.meteorTextureAssignment = Object.assign({}, entry.textureAssignment);
      if (entry.glb?.userData) entry.glb.userData.meteorTextureAssignment = Object.assign({}, entry.textureAssignment);
      emitMeteorTextureDebugEvent("world.meteor_texture_assigned", { visualId: entry.visualId, colorKey, mapUrl: entry.textureAssignment.mapUrl, emissiveMapUrl: entry.textureAssignment.emissiveMapUrl });
    } else {
      if (entry.meteorTextureRestorePending) restoreMeteorTextureStateForEntry(entry);
      entry.meteorTextureAssignments = null;
      entry.textureAssignment = null;
      entry.root.userData.meteorTextureAssignment = null;
      entry.meteorTextureAppliedUrls = { map: null, emissiveMap: null };
      entry.meteorTextureApplySignature = null;
    ...
    entry.assetPath = assignment.path;
  ...
  function getAsteroidGlbAssetPath(asteroid) {
    return ASTEROID_GLB_ASSETS[normalizeAsteroidVisualVariant(asteroid)];
  }

  function getAsteroidGlbAssetUrl(asteroid) {
    return resolvePublicAssetPath(getAsteroidGlbAssetPath(asteroid));
  }
  ...
  function getPlanetGlbAssetPath(planet) {
    return PLANET_GLB_ASSETS[normalizePlanetVisualVariant(planet)];
  }
  ...
  function isThreeCameraModel(value) { return String(value) === "absolute_bounds"; }

  function getThreeCameraModel() {
    window.HC = window.HC || {};
    window.HC.WorldRendererDebug = window.HC.WorldRendererDebug || {};
    window.HC.WorldRendererDebug.cameraModel = "absolute_bounds";
    if (window.HC.Session?.debugConfig?.visual) window.HC.Session.debugConfig.visual.cameraModel = "absolute_bounds";
  ...
  function applyRenderSpaceToVector(sourceX, sourceY, z = 0) {
    return { x: Number(sourceX) || 0, y: Number(sourceY) || 0, z };
  }

  function applyRenderSpaceToRadius(radius) {
    return Math.max(0.0001, Number(radius) || 0);
  }
  ...
  function loadGlbWithGltfLoader(THREE, logicalPath, assetKind = "generic") {
    const resolvedUrl = resolvePublicAssetPath(logicalPath);
    let entry = threeState.glbTemplateCache.get(logicalPath);
    if (entry) {
      if (entry.status === "loading") syncGltfLoaderUrlDiagnostics();
      return entry;
    }
    const startedAt = performance.now();
    entry = { status: "loading", url: resolvedUrl, rawUrl: String(logicalPath || ""), logicalPath: String(logicalPath || ""), resolvedUrl, baseUrl: getBrowserAssetBaseUrl(), cacheKey: logicalPath, assetKind, loaderMode: "gltf_loader", loaderPrimary: "GLTFLoader", template: null, error: null, errorMessage: null, errorStack: null, startedAt, durationMs: null, timeoutMs: getGlbLoadTimeoutMs(assetKind), progressLoaded: null, progressTotal: null, progressEventEmitted: false, materialAudit: null, promise: null };
    threeState.glbTemplateCache.set(logicalPath, entry);
    entry.promise = parseGlbWithGltfLoader(THREE, resolvedUrl, assetKind, entry)
      .then((template) => {
        entry.template = template;
        entry.materialAudit = collectGlbMaterialAudit(template, resolvedUrl);
        logGlbMaterialAudit();
        entry.status = "ready";
        return template;
      })
      .catch((error) => {
        ...
        removeFromList(diagnostics.gltfLoaderPendingUrls, resolvedUrl);
        syncGltfLoaderUrlDiagnostics();
        if (!error?.hcGlbTimeout && !error?.hcGlbEventEmitted) emitGlbDebugEvent("world.glb_load_failed", makeGlbEventPayload("failed", { assetKind, logicalPath, url: resolvedUrl, cacheKey: entry.cacheKey, durationMs: entry.durationMs, loaded: entry.progressLoaded, total: entry.progressTotal, error, status: "failed" }));
        warnGlbLoadFailureOnce(resolvedUrl, entry.error, { assetKind, durationMs: entry.durationMs, progressLoaded: entry.progressLoaded, progressTotal: entry.progressTotal });
        if (assetKind === "asteroid") {
          threeState.asteroidGlbTextureDiagnostics = Object.assign(createAsteroidGlbTextureDiagnostics(), {
            asteroidGlbLoaderMode: "gltf_loader",
            lastAssetUrl: resolvedUrl,
  ...
  function loadMeteorGlb(THREE, logicalPath) {
    const entry = loadGlbWithGltfLoader(THREE, logicalPath, "meteor");
    threeState.meteorGlbCache.set(logicalPath, entry);
    if (entry.status === "failed") warnMeteorGlbOnce(entry.resolvedUrl, entry.error || "GLTFLoader failed");
    return entry;
  }
  ...
  function createMeteorVisual(THREE, colorKey) {
    ...
    const entry = { root, fallback, glb: null, visualId, colorKey: null, meteorColorKey: colorKey, sourceMeteor: null, meteor: null, assetPath: null, assetUrl: null, variantIndex: null, glbStatus: "fallback", rotationState, meteorTextureAssignments: null, meteorTextureAppliedUrls: { map: null, emissiveMap: null }, meteorTextureApplySignature: null, meteorTextureLastMaterialMode: null, meteorTextureLastToggleEnabled: null, meteorTextureStatus: "idle", meteorTextureRestorePending: false };
  ...
  function updateMeteorGlbVisual(THREE, entry) {
    ...
    const logicalPath = entry.assetPath || null;
    ...
    const cacheEntry = loadMeteorGlb(THREE, logicalPath);
  ...
  function loadAsteroidGlb(THREE, logicalPath) {
    const entry = loadGlbWithGltfLoader(THREE, logicalPath, "asteroid");
    threeState.asteroidGlbCache.set(logicalPath, entry);
    if (entry.status === "failed") warnAsteroidGlbOnce(entry.resolvedUrl, entry.error || "GLTFLoader failed");
    return entry;
  }
  ...
  function createAsteroidVisual(THREE, asteroid, key) {
    ...
    root.userData.glbAssetPath = getAsteroidGlbAssetPath(asteroid);
    ...
    root.userData.rotationSpeed = rotationState.rotationSpeed;
    root.userData.rotationDominantAxis = rotationState.dominantAxis;
    return { root, fallback, glb: null, key, assetPath: getAsteroidGlbAssetPath(asteroid), assetUrl: getAsteroidGlbAssetUrl(asteroid), visualVariant: normalizeAsteroidVisualVariant(asteroid), glbStatus: "assigned", rotationState, asteroid: null };
  }
  ...
  function updateAsteroidGlbVisual(THREE, entry) {
    if (!entry) return false;
    const logicalPath = entry.assetPath || getAsteroidGlbAssetPath(entry.asteroid);
    const assetUrl = entry.assetUrl || resolvePublicAssetPath(logicalPath);
    const cacheEntry = loadAsteroidGlb(THREE, logicalPath);
    if (cacheEntry.status === "failed") {
      entry.glbStatus = "failed";
      entry.root.userData.glbStatus = entry.glbStatus;
      entry.fallback.visible = true;
  ...
  function initThree() {
    ...
    try {
      ...
      cosmicDustGroup.name = "hc_cosmic_dust_group";
      ...
      Object.assign(threeState, { canvas, renderer, scene, camera, orthographicCamera, perspectiveCamera, meteorGroup, asteroidGroup, planetGroup, moonGroup, harmonicDustGroup, cosmicDustGroup, prgIndicatorGroup, lightsGroup, meteorGeometry: new THREE.CircleGeometry(1, 16), planetGeometry: new THREE.CircleGeometry(1, 32), harmonicDustGeometry: new THREE.CircleGeometry(1, 24), cosmicDustGeometry: new THREE.CircleGeometry(1, 20) });
      applyRendererPbrSettings();
      syncThreeLights();
      ...
      preloadMeteorTexturePalettes(THREE);
  ...
  function getCosmicDustMaterial(THREE) {
    if (threeState.cosmicDustMaterial) return threeState.cosmicDustMaterial;
    threeState.cosmicDustMaterial = new THREE.MeshBasicMaterial({
      color: new THREE.Color(0x8d9092),
      transparent: true,
      opacity: 0.26,
      side: THREE.DoubleSide,
      depthTest: false,
    ...
    return threeState.cosmicDustMaterial;
  }

  function getStableCosmicDustSnapshotKey(dust, index) {
    const key = dust?.renderKey || dust?.id || dust?._id || null;
    if (key != null && key !== "") return `cosmic_dust:${key}`;
    return `cosmic_dust:${index}:${Math.round(Number(dust?.x) || 0)}:${Math.round(Number(dust?.y) || 0)}`;
  }

  function createCosmicDustVisual(THREE, dust, key) {
    const root = new THREE.Group();
    root.userData.hcObjectType = "cosmic_dust";
    root.userData.cosmicDustKey = key;
    root.frustumCulled = false;
    const mesh = new THREE.Mesh(threeState.cosmicDustGeometry || threeState.harmonicDustGeometry, getCosmicDustMaterial(THREE));
    mesh.frustumCulled = false;
  ...
  function syncCosmicDustPass(renderSnapshot) {
    const THREE = window.HC_THREE || window.THREE;
    const samples = Array.isArray(renderSnapshot?.world?.cosmicDust) ? renderSnapshot.world.cosmicDust : [];
    if (!THREE || !threeState.cosmicDustGroup) return;
    const seen = new Set();
    for (let i = 0; i < samples.length; i += 1) {
      const dust = samples[i] || {};
      const key = getStableCosmicDustSnapshotKey(dust, i);
      seen.add(key);
      let visual = threeState.cosmicDustMeshes.get(key);
      if (!visual) {
        visual = createCosmicDustVisual(THREE, dust, key);
        threeState.cosmicDustGroup.add(visual.root);
        threeState.cosmicDustMeshes.set(key, visual);
      }
      const radius = Math.max(1, Number(dust.visual?.radius ?? dust.r) || 1);
      const renderRadius = applyRenderSpaceToRadius(radius);
      const pos = applyRenderSpaceToVector(Number(dust.x) || 0, Number(dust.y) || 0, Number(dust.z) || 0.035);
      ...
      visual.mesh.material.opacity = Math.max(0.08, Math.min(0.38, Number(dust.visual?.alpha) || 0.24));
    }
    for (const [key, visual] of threeState.cosmicDustMeshes.entries()) {
      if (seen.has(key)) continue;
      threeState.cosmicDustGroup.remove(visual.root);
      threeState.cosmicDustMeshes.delete(key);
    }
    threeState.cosmicDustGroupChildrenCount = threeState.cosmicDustGroup.children.length;
  }
  ...
  function syncAsteroidPass(renderSnapshot, nowMs) {
    ...
    for (let i = 0; i < asteroids.length; i += 1) {
      ...
      if (visual.assetUrl !== desiredAssetUrl) {
        ...
        visual.assetPath = getAsteroidGlbAssetPath(a);
        visual.assetUrl = desiredAssetUrl;
        visual.visualVariant = desiredVariant;
        visual.root.userData.glbAssetPath = visual.assetPath;
  ...
  function createPlanetVisual(THREE, planet, key) {
    ...
    const assetPath = getPlanetGlbAssetPath(planet);
    ...
    root.add(fallback);
    return { root, fallback, glb: null, key, assetPath, assetUrl, visualVariant, glbStatus: "assigned", planet, rotationStartMs: null };
  }
  ...
  function updatePlanetGlbVisual(THREE, entry) {
    const cacheEntry = loadGlbWithGltfLoader(THREE, entry.assetPath, "planet");
    threeState.planetGlbCache.set(entry.assetPath, cacheEntry);
    if (cacheEntry.status === "failed") {
      entry.glbStatus = "failed";
      entry.root.userData.glbStatus = entry.glbStatus;
      entry.fallback.visible = true;
  ...
  function destroyThree() {
    ...
    threeState.cosmicDustMeshes.forEach((entry) => { threeState.cosmicDustGroup?.remove(entry?.root || entry); });
    threeState.cosmicDustMeshes.clear();
    ...
    threeState.cosmicDustGeometry?.dispose?.();
    threeState.cosmicDustMaterial?.dispose?.();
    threeState.prgIndicatorGeometry?.dispose?.();
  ...
  function buildMinimalDiagnosticsSnapshotFallback(reason) {
    return {
      requestedMode,
      effectiveMode,
      initialized,
      ...
      renderCalls,
      fallbackCalls,
      threeModeActive: effectiveMode === "three",
      threeDependencySource,
      threeState: {
        initialized: !!threeState.initialized,
        hasDependency: !!threeState.hasDependency,
        lastError: threeState.lastError || null,
        renderCalls: threeState.renderCalls || 0,
        resizeCalls: threeState.resizeCalls || 0,
      },
      diagnosticsSnapshotFallbackUsed: true,
      diagnosticsSnapshotFallbackReason: reason || "missing_world_renderer_diagnostics_snapshot_helper",
    };
  }

  function getDiagnostics() {
    const snapshotApi = window.HC?.WorldRendererDiagnosticsSnapshot;
    if (!snapshotApi || typeof snapshotApi.buildWorldRendererDiagnosticsSnapshot !== "function") {
      return buildMinimalDiagnosticsSnapshotFallback("missing_buildWorldRendererDiagnosticsSnapshot");
    }
    try {
      const gltfLoaderDiagnostics = syncGltfLoaderUrlDiagnostics();
      refreshGltfLoaderAvailabilityDiagnostics();
      return snapshotApi.buildWorldRendererDiagnosticsSnapshot({
        requestedMode, effectiveMode, fallbackUsed, fallbackReason, lastError, initialized, renderCalls, fallbackCalls, threeDependencySource,
        threeState,
        constants: { METEOR_GLB_ASSETS, ASTEROID_GLB_ASSETS, ASTEROID_GLB_DEFAULT_VARIANT, PLANET_GLB_ASSETS, PLANET_GLB_DEFAULT_VARIANT, ASTEROID_GLB_RADIUS_SCALE, MOON_GLB_ASSETS },
        ...
        fns: {
          getCanvasDiagnostics, getGameCanvasDiagnostics, getLayerProbe, getThreeLightsSettings, getLightDistanceDiagnostics,
          roundDiagnosticNumber, vectorToDiagnostic, getStageLightCounts, createGltfDebugProbeDiagnostics, resolvePublicAssetPath,
          getAsteroidGlbAssetUrl, getPlanetGlbAssetUrl, getAsteroidGlbCacheStats, summarizeGlbCacheEntries, getPlanetGlbAssetPath,
          createAsteroidGlbTextureDiagnostics, getMeteorGlbVisualScale, getMeteorGlbDepthScale, getGlobalHelpersEnabled, getThreeMaterialSettings,
          buildMeteorTexturePalette, getMeteorTexturePassEnabled, isMeteorTextureColorEnabled, getMeteorTextureCacheStats, countMeteorTextureUsage,
          countMeteorTextureAssignmentsByColor, getMeteorTextureReadinessByColor, getMeteorTextureEvidence, getMaterialAuditOverlayStatus,
          getMeteorGlbCacheStats, countMeteorVisualsByColor, getSceneFrameVisible,
        },
      });
    } catch (error) {
      return buildMinimalDiagnosticsSnapshotFallback(error?.message || "buildWorldRendererDiagnosticsSnapshot_failed");
    }
  }

  function getThreeAssetUrlContract() {
    const rows = [
      { kind: "meteorGlb", logicalPath: METEOR_GLB_ASSETS.red[0] },
      { kind: "meteorTexture", logicalPath: METEOR_TEXTURE_PALETTES.red.map[0] },
      { kind: "meteorEmissiveMap", logicalPath: METEOR_TEXTURE_PALETTES.red.emissiveMap[0] },
      { kind: "asteroidGlb", logicalPath: ASTEROID_GLB_ASSETS[ASTEROID_GLB_DEFAULT_VARIANT] },
    ];
    return rows.map((row) => Object.assign({}, row, { resolvedUrl: resolvePublicAssetPath(row.logicalPath) }));
  }

  function destroy() { destroyThree(); initialized = false; resetDiagnostics(); }

  window.HC.WorldRenderer = { init, resize, render, destroy, getDiagnostics, setMode, getMode, isPlanetVisualCandidate, getMeteorGlbVisualScale, setMeteorGlbVisualScale, getMeteorGlbDepthScale, setMeteorGlbDepthScale, getThreeCameraModel, setThreeCameraModel, getGlobalHelpersEnabled, setGlobalHelpersEnabled, getThreeLightsSettings, setThreeLightsDebugSetting, getThreeMaterialSettings, setThreeMaterialDebugSetting, getThreeAssetUrlContract };
})();
