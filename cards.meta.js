console.log("[HC] cards.meta.js loaded");
/* =========================================================
...
const __CardMetaEngine = (() => {
  ...
  let _cardSequencesApi = null;
  function getCardSequencesApi() {
    if (_cardSequencesApi) return _cardSequencesApi;
    const factory = (typeof window !== "undefined" && window.CardSequences && window.CardSequences.create)
      ? window.CardSequences.create
      : null;
    if (typeof factory !== "function") return null;
    _cardSequencesApi = factory({
      state,
      config,
      CardRegistry,
      nowMs,
  ...
  function traceSeqHit(action, normalizedColor, details) {
    const api = getCardSequencesApi();
    if (!api || typeof api.traceSeqHit !== "function") return;
    return api.traceSeqHit(action, normalizedColor, details);
  }

  function showSequenceOverlay(level, colors, colorKey, ttlMs) {
    const api = getCardSequencesApi();
    if (!api || typeof api.showSequenceOverlay !== "function") return;
    return api.showSequenceOverlay.apply(null, arguments);
  }

  function showSequenceToast(title, subtitle, colorKey, ttlMs) {
    const api = getCardSequencesApi();
    if (!api || typeof api.showSequenceToast !== "function") return;
    return api.showSequenceToast.apply(null, arguments);
  }

  function showSequenceFailToast(pointsLabel, colors, ttlMs) {
    const api = getCardSequencesApi();
    if (!api || typeof api.showSequenceFailToast !== "function") return;
    return api.showSequenceFailToast.apply(null, arguments);
  }
  ...
  function startSequenceSession() {
    const api = getCardSequencesApi();
    if (!api || typeof api.startSequenceSession !== "function") return;
    return api.startSequenceSession();
  }
  ...
  function awardSequenceStep(World, seq) {
    const api = getCardSequencesApi();
    if (!api || typeof api.awardSequenceStep !== "function") return null;
    return api.awardSequenceStep(World, seq);
  }

  function closeSequenceStep(World) {
    const api = getCardSequencesApi();
    if (!api || typeof api.closeSequenceStep !== "function") return;
    return api.closeSequenceStep(World);
  }
  ...
  function failSequence(World) {
    const api = getCardSequencesApi();
    if (!api || typeof api.failSequence !== "function") return;
    return api.failSequence(World);
  }

  function finalizeSequence(World, level, colors, rewardCard) {
    const api = getCardSequencesApi();
    if (!api || typeof api.finalizeSequence !== "function") return;
    return api.finalizeSequence(World, level, colors, rewardCard);
  }

  function onHitColor(colorKey) {
    const api = getCardSequencesApi();
    if (!api || typeof api.onHitColor !== "function") return { action: "ignored", snapshot: null };
    return api.onHitColor(colorKey);
  }
  ...
  function activateSequenceR1(colorKey, options = {}) {
    const api = getCardSequencesApi();
    if (!api || typeof api.activateSequenceR1 !== "function") return false;
    return api.activateSequenceR1(colorKey, options);
  }

  function cashOutSequence(level, colors) {
    const api = getCardSequencesApi();
    if (!api || typeof api.cashOutSequence !== "function") return false;
    return api.cashOutSequence(level, colors);
  }
  ...
  function onRunActivateR1({ baseDurationMs, colorKey, tierKey, keepSequence } = {}) {
    const api = getCardSequencesApi();
    if (!api || typeof api.onRunActivateR1 !== "function") return false;
    return api.onRunActivateR1({ baseDurationMs, colorKey, tierKey, keepSequence });
  }
  ...
  function setPendingChoiceCard(World, payload) {
    const api = getCardSequencesApi();
    if (!api || typeof api.setPendingChoiceCard !== "function") return null;
    return api.setPendingChoiceCard(World, payload);
  }

  function flushPendingCard(World, nowMs) {
    const api = getCardSequencesApi();
    if (!api || typeof api.flushPendingCard !== "function") return false;
    return api.flushPendingCard(World, nowMs);
  }

  function consumePendingCard(World, { kind, tier, colors } = {}, nowMs) {
    const api = getCardSequencesApi();
    if (!api || typeof api.consumePendingCard !== "function") return false;
    return api.consumePendingCard(World, { kind, tier, colors }, nowMs);
  }
...
var CardMeta = (typeof window !== "undefined" && window.CardMeta)
  ? window.CardMeta
  : {
    create: () => __CardMetaEngine
  };

if (typeof window !== "undefined") {
  window.CardMeta = window.CardMeta || CardMeta;
  window.HC = window.HC || {};
  if (!window.HC.seqSim) window.HC.seqSim = __CardMetaEngine.seqSim || null;
  if (!window.HC.seqSimTests) window.HC.seqSimTests = __CardMetaEngine.seqSimTests || null;
  window.HC_SEQ_PROBE = window.HC_SEQ_PROBE || {};
  window.HC_SEQ_PROBE.simAAA = __CardMetaEngine.seqProbeSimAAA || null;
}
