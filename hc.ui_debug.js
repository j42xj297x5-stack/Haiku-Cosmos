...
(function () {
  ...
  function getThreeLightsSettingsForUi() {
    ...
    const defaults = { enabled: true, ambientIntensity: 0.10, ambientIsolate: false, mainStageSpotEnabled: true, mainStageSpotIntensity: 4.2, mainStageSpotAngle: Math.PI / 2.8, mainStageSpotPenumbra: 0.72, mainStageSpotDistance: 0, mainStageSpotDecay: 0, mainStageSpotXOffset: -0.35, mainStageSpotYOffset: 0.35, mainStageSpotZHeight: 0.75, showLightHelpers: false, showSceneFrame: false };
  ...
  function setThreeLightsSettingFromUi(key, value) {
    ...
    const inputMap = {
      ...
      mainStageSpotEnabled: "dbgThreeMainStageSpotEnabled",
      mainStageSpotIntensity: "dbgThreeMainStageSpotIntensity",
      mainStageSpotAngle: "dbgThreeMainStageSpotAngle",
      mainStageSpotPenumbra: "dbgThreeMainStageSpotPenumbra",
      mainStageSpotXOffset: "dbgThreeMainStageSpotXOffset",
      mainStageSpotYOffset: "dbgThreeMainStageSpotYOffset",
      mainStageSpotZHeight: "dbgThreeMainStageSpotZHeight",
      showLightHelpers: "dbgThreeShowLightHelpers",
      showSceneFrame: "dbgThreeShowSceneFrame",
    };
  ...
  function renderRuntimeOverlayHtml(snap, compact) {
    ...
    sections.push(renderSection("lighting", "Lighting", [
      ...
      ["compact status", rendererDiag?.mainStageSpot ? "mainStageSpot active" : "no spot diagnostics"],
    ], `
    ...
    sections.push(renderSection("lighting-advanced", "Lighting Advanced", [
      ["lighting model", rendererDiag?.lightingModelVersion || rendererDiag?.stageLighting?.lightingModelVersion || "stage_spot_v1_absolute"],
      ...
      <label class="overlay-select-row" for="dbgThreeMainStageSpotXOffset">Spot X offset <input id="dbgThreeMainStageSpotXOffset" type="range" min="-1.5" max="1.5" step="0.05" value="${threeLights.mainStageSpotXOffset}"><span id="dbgThreeMainStageSpotXOffsetValue">${Number(threeLights.mainStageSpotXOffset).toFixed(2)}</span></label>
      <label class="overlay-select-row" for="dbgThreeMainStageSpotYOffset">Spot Y offset <input id="dbgThreeMainStageSpotYOffset" type="range" min="-1.5" max="1.5" step="0.05" value="${threeLights.mainStageSpotYOffset}"><span id="dbgThreeMainStageSpotYOffsetValue">${Number(threeLights.mainStageSpotYOffset).toFixed(2)}</span></label>
      <label class="overlay-select-row" for="dbgThreeMainStageSpotZHeight">Spot Z height <input id="dbgThreeMainStageSpotZHeight" type="range" min="0.10" max="3.0" step="0.05" value="${threeLights.mainStageSpotZHeight}"><span id="dbgThreeMainStageSpotZHeightValue">${Number(threeLights.mainStageSpotZHeight).toFixed(2)}</span></label>
    ...
    sections.push(renderSection("world-mechanics", "World / Mechanics", [
      ...
      ["live M+A collisions", window.World?.liveMeteorAsteroidCollisionCount ?? 0],
      ...
      ["cosmic visual", window.World?.spaceMechanics?.cosmicDustVisualEnabled !== false ? "on" : "off"],
      ["cosmic affects", window.World?.spaceMechanics?.cosmicDustAffectsBodiesEnabled === true ? "on" : "off"],
      ["cosmic affected/stopped", `${wc.cosmicDustAffectedBodiesCount ?? window.World?.cosmicDustAffectedBodiesCount ?? 0}/${wc.cosmicDustStoppedBodiesCount ?? window.World?.cosmicDustStoppedBodiesCount ?? 0}`],
      ["cosmic drag", Number(window.World?.spaceMechanics?.cosmicDustDragStrength || 0).toFixed(3)],
      ["cosmic stop speed", Number(window.World?.spaceMechanics?.cosmicDustStopSpeedThreshold || 0).toFixed(3)],
      ["cosmic mass", Number(wc.cosmicDustTotalMass || 0).toFixed(2)],
      ...
      ["dust body xform", window.World?.spaceMechanics?.harmonicDustBodyTransformEnabled !== false ? "on" : "off"],
      ["asteroid grays dust", window.World?.spaceMechanics?.harmonicDustAsteroidGrayEnabled !== false ? "on" : "off"],
      ["moon ring absorb", window.World?.spaceMechanics?.harmonicDustMoonRingAbsorbEnabled !== false ? "on" : "off"],
      ["gray world dust", Array.isArray(window.World?.harmonicDust) ? window.World.harmonicDust.filter((dust) => dust && !dust._dead && (dust.colorName === "GRAY" || Number(dust.grayMixRatio) > 0)).length : 0],
      ["elastic gray shift", window.World?.spaceMechanics?.harmonicDustElasticGrayEnabled !== false ? "on" : "off"],
      ["gray shifting/recover/lock", Array.isArray(window.World?.harmonicDust) ? `${window.World.harmonicDust.filter((dust) => dust && !dust._dead && dust.transformState === "gray_shifting").length}/${window.World.harmonicDust.filter((dust) => dust && !dust._dead && dust.transformState === "recovering").length}/${window.World.harmonicDust.filter((dust) => dust && !dust._dead && dust.transformState === "gray_locked").length}` : "0/0/0"],
      ["max gray mix", Array.isArray(window.World?.harmonicDust) ? Math.max(0, ...window.World.harmonicDust.filter((dust) => dust && !dust._dead).map((dust) => Number(dust.grayMixRatio) || 0)).toFixed(2) : "0.00"],
      ["moons w dust rings", Array.isArray(window.World?.moons) ? window.World.moons.filter((moon) => Array.isArray(moon?.dustRings) && moon.dustRings.length > 0).length : 0],
