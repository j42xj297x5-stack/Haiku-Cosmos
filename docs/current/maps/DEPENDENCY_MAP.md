...
- `WORLD_RENDERING_MODEL.md` definiuje roboczy kontrakt migracji renderingu świata (Three.js adapter + feature flag + fallback), bez zmian mechaniki/ekonomii/sekwencji/UI overlay; zawiera też adapter bootstrap contract (Etap 0.75), snapshot Three GLB meteor pass v0.1 oraz snapshot Three GLB PBR material pipeline po naprawie.
- Runtime relacja assetów GLB: `hc.world_renderer.js` korzysta z aktywnych puli meteorów w `public/glb/` przez `publicAssetPath` / `publicPath`; assety GLB są warstwą prezentacyjną Three i nie są zależnością mechaniki, kolizji, kart, RP, HUD ani SUB-META. Aktualne GLB mają PBR factory materiały, lecz 0 tekstur i 0 normalMap; widoczny detal powierzchni wymaga kolejnego bake/eksport passu.
