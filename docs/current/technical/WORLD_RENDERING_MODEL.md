...
## 18. GitHub Pages GLB dependency routing checkpoint (2026-06-06)

- Poprawny bezpośredni URL pliku `.glb` potwierdza wyłącznie dostępność kontenera. `GLTFLoader` może nadal zakończyć się błędem podczas pobierania URI obrazów, buforów albo innych zależności parsera.
- Aktywny loader tworzy osobny `THREE.LoadingManager`, ustawia `resourcePath` na katalog **rozwiązanego** URL-u modelu oraz przepuszcza URL-e parsera przez base-safe `setURLModifier`. Ścieżki zaczynające się od `/assets/`, `/glb/`, `/models/`, `/png/`, `/svg/` lub `/textures/` są na GitHub Pages przepisywane przez `HC.publicAssetPath`, a błędny segment `/public/` jest usuwany. URL-e `blob:`, `data:` i zasoby cross-origin pozostają bez zmian.
- Diagnostyka `HC.WorldRenderer.getDiagnostics()` raportuje teraz `gltfLoaderLastResourcePath`, liczniki LoadingManagera, ostatnie URL-e start/complete/error oraz listy `gltfDependencyRequestedUrls`, `gltfDependencyCompletedUrls` i `gltfFailedDependencyUrls`. Zdarzenia runtime obejmują `world.glb_dependency_started`, `world.glb_dependency_ready` i `world.glb_dependency_failed`.
