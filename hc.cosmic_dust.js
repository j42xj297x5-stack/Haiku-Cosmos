...
(function () {
  ...
  function attachRuleEvidence(evidence, world, rule) { if (!rule) return evidence; evidence.ruleId = rule.id || evidence.ruleId; evidence.ruleSource = world?.collisionRulesDiagnostics?.collisionRulesSource || "runtime_table"; evidence.sourceMassPolicy = rule.sourceMassPolicy || null; evidence.ruleVersion = world?.collisionRules?.version || window.HC?.CollisionRules?._active?.version || 1; evidence.dustPct = Math.max(0, Math.min(1, finite(rule.dustPct, 0))); evidence.absorbPct = Math.max(0, Math.min(1, finite(rule.absorbPct, 0))); evidence.fragmentsPct = Math.max(0, Math.min(1, finite(rule.fragmentsPct, 0))); evidence.orbiterPct = Math.max(0, Math.min(1, finite(rule.orbiterPct, 0))); evidence.rulePct = { dustPct: evidence.dustPct, absorbPct: evidence.absorbPct, fragmentsPct: evidence.fragmentsPct, orbiterPct: evidence.orbiterPct }; return evidence; }
  ...
  function ensureWorldState(World) {
    ...
    if (!Array.isArray(World.impactFragmentDescriptors)) World.impactFragmentDescriptors = [];
    if (!Array.isArray(World.orbiterCandidateDescriptors)) World.orbiterCandidateDescriptors = [];
  ...
  function addMass(body, amount, kind, sourceFunction) { if (body && amount > 0) { body.mass = bodyMass(body) + amount; refreshRadius(body, kind, sourceFunction); if (kind === "asteroid") window.HC?.Asteroids?.checkAsteroidMoonThreshold?.(body, sourceFunction); if (kind === "moon") window.HC?.Asteroids?.checkMoonRockyPlanetThreshold?.(body, sourceFunction); } }
  function recordMassSplit(world, evidence, inputMass, outputOverride) {
    const output = Number.isFinite(Number(outputOverride)) ? Number(outputOverride) : ((evidence.dustMass ?? evidence.cosmicDustMass ?? 0) + (evidence.absorbMass ?? evidence.absorbedMass ?? 0) + (evidence.fragmentsMass ?? evidence.fragmentMass ?? 0) + (evidence.orbiterMass ?? evidence.orbiterCandidate?.mass ?? 0));
  ...
  function createImpactFragmentDescriptor(world, mass, target, incoming, rule, before, after) {
    const descriptor = Object.assign({
      id: `impact_fragment_descriptor:${(world.impactFragmentDescriptors?.length || 0) + 1}`,
      type: "impact_fragment_descriptor_created",
      kind: "impactFragmentDescriptor",
      sourceRuleId: rule?.id || null,
      sourceBodyIds: [target, incoming].map(bodyId).filter(Boolean),
      sourceMass: Math.max(0, before || 0),
      fragmentMass: Math.max(0, mass),
      createdAtFrame: Number(world.frame) || 0,
      createdAtSessionTimeMs: nowMs(world),
      runtimeBody: false,
      implementedMovement: false,
    }, sourceEvidence("impact_fragment_descriptor", rule, "HC.CosmicDust.applySplitPolicy", [target, incoming], before, after));
    world.impactFragmentDescriptors.push(descriptor);
    world.lastImpactFragmentDescriptor = descriptor;
    window.HC?.logEvent?.("world", "impact_fragment_descriptor_created", descriptor, { source: "HC.CosmicDust.applySplitPolicy", snapshot: true });
    return descriptor;
  }
  function createOrbiterCandidateDescriptor(world, mass, host, incoming, rule, before, after) {
    const descriptor = Object.assign({
      id: `orbiter_candidate_descriptor:${(world.orbiterCandidateDescriptors?.length || 0) + 1}`,
      type: "orbiter_candidate_descriptor_created",
      kind: "orbiterCandidateDescriptor",
      sourceRuleId: rule?.id || null,
      sourceBodyIds: [host, incoming].map(bodyId).filter(Boolean),
      hostBodyId: bodyId(host),
      hostBodyKind: bodyKind(host),
      sourceMass: Math.max(0, before || 0),
      orbiterMass: Math.max(0, mass),
      mass: Math.max(0, mass),
      sourcePath: "orbital_asteroid_candidate",
      createdAtFrame: Number(world.frame) || 0,
      createdAtSessionTimeMs: nowMs(world),
      runtimeBody: false,
      implementedOrbit: false,
    }, sourceEvidence("orbiter_candidate_descriptor", rule, "HC.CosmicDust.applySplitPolicy", [host, incoming], before, after));
    descriptor.sourcePath = "orbital_asteroid_candidate";
    world.orbiterCandidateDescriptors.push(descriptor);
    world.lastOrbiterCandidateDescriptor = descriptor;
    window.HC?.logEvent?.("world", "orbiter_candidate_descriptor_created", descriptor, { source: "HC.CosmicDust.applySplitPolicy", snapshot: true });
    return descriptor;
  }
  ...
  function hasValidPlanetOrigin(planet) {
    ...
    return Boolean(planet.sourcePath && planet.sourceFunction);
  }
  function blockInvalidPlanetImpact(world, kind, planet) {
    if (!planet) return false;
    if (hasValidPlanetOrigin(planet)) return false;
    planet.invalidPlanetOrigin = true;
    world.invalidPlanetOriginCount = (Number(world.invalidPlanetOriginCount) || 0) + 1;
    world.invalidPlanetOriginSamples = Array.isArray(world.invalidPlanetOriginSamples) ? world.invalidPlanetOriginSamples : [];
    const event = {
      type: "invalid_planet_origin",
      collisionKind: kind,
      planetId: bodyId(planet),
      planetKind: planet.planetKind || null,
      ...
      blockedSourceFunction: "HC.CosmicDust.applySplitPolicy",
      blockedSourcePath: kind,
    };
    world.lastInvalidPlanetOriginEvent = event;
    world.invalidPlanetOriginSamples.push(event);
    while (world.invalidPlanetOriginSamples.length > 8) world.invalidPlanetOriginSamples.shift();
    window.HC?.logEvent?.("world", "invalid_planet_origin", event, { source: "HC.CosmicDust.applySplitPolicy", snapshot: true });
    return true;
  }
  function applySplitPolicy(World, spec) {
    ...
    else if (kind === "moon_meteor") { const moon = s.moon || a; const meteor = s.meteor || b; const m = bodyMass(meteor); const dustPct = rulePct(world, rule, "dustPct", "cosmicDustSplitMoonMeteorDustPct"); const absorbPct = rulePct(world, rule, "absorbPct", "cosmicDustSplitMoonMeteorAbsorbPct"); const fragmentsPct = rulePct(world, rule, "fragmentsPct", "cosmicDustSplitMoonAsteroidFragmentsPct"); const orbiterPct = rulePct(world, rule, "orbiterPct", "cosmicDustSplitPlanetAsteroidOrbiterPct"); evidence.sourceMass = m; evidence.dustPct = dustPct; evidence.absorbPct = absorbPct; evidence.fragmentsPct = fragmentsPct; evidence.orbiterPct = orbiterPct; evidence.dustMass = m * dustPct; evidence.absorbMass = m * absorbPct; evidence.fragmentsMass = m * fragmentsPct; evidence.orbiterMass = m * orbiterPct; dust(evidence.dustMass, moon, meteor); evidence.absorbedMass = evidence.absorbMass; evidence.fragmentMass = evidence.fragmentsMass; evidence.fragmentsDescriptor = evidence.fragmentsMass > 0 ? createImpactFragmentDescriptor(world, evidence.fragmentsMass, moon, meteor, rule, m, bodyMass(moon)) : null; evidence.orbiterDescriptor = evidence.orbiterMass > 0 ? createOrbiterCandidateDescriptor(world, evidence.orbiterMass, moon, meteor, rule, m, bodyMass(moon)) : null; addMass(moon, evidence.absorbedMass, "moon", "HC.CosmicDust.applySplitPolicy:moon_meteor"); resizeMoon(world, moon); if (meteor) meteor._dead = true; }
    else if (kind === "moon_asteroid") { const moon = s.moon || a; const asteroid = s.asteroid || b; const m = bodyMass(asteroid); dust(m * rulePct(world, rule, "dustPct", "cosmicDustSplitMoonAsteroidDustPct"), moon, asteroid); evidence.absorbedMass = m * rulePct(world, rule, "absorbPct", "cosmicDustSplitMoonAsteroidAbsorbPct"); evidence.fragmentMass = evidence.fragmentsMass = m * rulePct(world, rule, "fragmentsPct", "cosmicDustSplitMoonAsteroidFragmentsPct"); evidence.orbiterMass = m * rulePct(world, rule, "orbiterPct", "cosmicDustSplitPlanetAsteroidOrbiterPct"); addMass(moon, evidence.absorbedMass, "moon", "HC.CosmicDust.applySplitPolicy:moon_asteroid"); resizeMoon(world, moon); evidence.fragmentsDescriptor = evidence.fragmentsMass > 0 ? createImpactFragmentDescriptor(world, evidence.fragmentsMass, moon, asteroid, rule, m, bodyMass(moon)) : null; evidence.orbiterDescriptor = evidence.orbiterMass > 0 ? createOrbiterCandidateDescriptor(world, evidence.orbiterMass, moon, asteroid, rule, m, bodyMass(moon)) : null; evidence.fragments = []; if (asteroid) asteroid._dead = true; }
    else if (kind === "planet_meteor") { const planet = s.planet || a; if (blockInvalidPlanetImpact(world, kind, planet)) return Object.assign(evidence, { blocked: true, invalidPlanetOrigin: true }); const meteor = s.meteor || b; const m = bodyMass(meteor); dust(m * rulePct(world, rule, "dustPct", "cosmicDustSplitPlanetMeteorDustPct"), planet, meteor); evidence.absorbedMass = m * rulePct(world, rule, "absorbPct", "cosmicDustSplitPlanetMeteorAbsorbPct"); evidence.fragmentMass = evidence.fragmentsMass = m * rulePct(world, rule, "fragmentsPct", "cosmicDustSplitPlanetMeteorFragmentsPct"); evidence.orbiterMass = m * rulePct(world, rule, "orbiterPct", "cosmicDustSplitPlanetAsteroidOrbiterPct"); addMass(planet, evidence.absorbedMass, "planet", "HC.CosmicDust.applySplitPolicy:planet_meteor"); evidence.fragmentsDescriptor = evidence.fragmentsMass > 0 ? createImpactFragmentDescriptor(world, evidence.fragmentsMass, planet, meteor, rule, m, bodyMass(planet)) : null; evidence.orbiterDescriptor = evidence.orbiterMass > 0 ? createOrbiterCandidateDescriptor(world, evidence.orbiterMass, planet, meteor, rule, m, bodyMass(planet)) : null; evidence.fragments = []; if (meteor) meteor._dead = true; }
    else if (kind === "planet_asteroid") { const planet = s.planet || a; if (blockInvalidPlanetImpact(world, kind, planet)) return Object.assign(evidence, { blocked: true, invalidPlanetOrigin: true }); const asteroid = s.asteroid || b; const m = bodyMass(asteroid); dust(m * rulePct(world, rule, "dustPct", "cosmicDustSplitPlanetAsteroidDustPct"), planet, asteroid); evidence.fragmentMass = evidence.fragmentsMass = m * rulePct(world, rule, "fragmentsPct", "cosmicDustSplitPlanetAsteroidFragmentsPct"); evidence.orbiterMass = m * rulePct(world, rule, "orbiterPct", "cosmicDustSplitPlanetAsteroidOrbiterPct"); evidence.fragmentsDescriptor = evidence.fragmentsMass > 0 ? createImpactFragmentDescriptor(world, evidence.fragmentsMass, planet, asteroid, rule, m, bodyMass(planet)) : null; evidence.orbiterCandidate = evidence.orbiterMass > 0 ? createOrbiterCandidateDescriptor(world, evidence.orbiterMass, planet, asteroid, rule, m, bodyMass(planet)) : null; evidence.fragments = []; if (asteroid) asteroid._dead = true; }
    else if (kind === "planet_moon") { const planet = s.planet || a; const moon = s.moon || b; const m = bodyMass(moon); dust(m * rulePct(world, rule, "dustPct", "cosmicDustSplitPlanetMoonDustPct"), planet, moon); evidence.fragmentMass = evidence.fragmentsMass = m * rulePct(world, rule, "fragmentsPct", "cosmicDustSplitPlanetMoonFragmentsPct"); evidence.orbiterMass = m * rulePct(world, rule, "orbiterPct", "cosmicDustSplitPlanetMoonOrbiterPct"); evidence.fragmentsDescriptor = evidence.fragmentsMass > 0 ? createImpactFragmentDescriptor(world, evidence.fragmentsMass, planet, moon, rule, m, bodyMass(planet)) : null; evidence.orbiterCandidate = evidence.orbiterMass > 0 ? createOrbiterCandidateDescriptor(world, evidence.orbiterMass, planet, moon, rule, m, bodyMass(planet)) : null; evidence.fragments = []; if (moon) moon._dead = true; }
    else if (kind === "planet_planet") { const m = bodyMass(a) + bodyMass(b); dust(m * rulePct(world, rule, "dustPct", "cosmicDustSplitPlanetPlanetDustPct"), a, b); evidence.fragmentMass = evidence.fragmentsMass = m * rulePct(world, rule, "fragmentsPct", "cosmicDustSplitPlanetPlanetFragmentsPct"); evidence.orbiterMass = 0; evidence.fragmentsDescriptor = evidence.fragmentsMass > 0 ? createImpactFragmentDescriptor(world, evidence.fragmentsMass, a, b, rule, m, 0) : null; evidence.fragments = []; if (a) a._dead = true; if (b) b._dead = true; evidence.descriptorOnly = true; }
    ...
    if (kind === "moon_meteor") world.lastMoonMeteorSplitEvent = Object.assign({ type: "moon_meteor" }, world.lastMassSplitEvent || evidence);
    if (evidence.cloud) {
      ...
      const compactPayload = Object.assign({
        ...
        rawRadiusFromMass: Number(targetBody?.lastRadiusRefresh?.rawRadiusFromMass ?? targetBody?.lastRadiusRefresh?.unclampedRadius) || null,
        unclampedRadius: Number(targetBody?.lastRadiusRefresh?.unclampedRadius ?? targetBody?.lastRadiusRefresh?.rawRadiusFromMass) || null,
        finalRadius: Number(targetBody?.lastRadiusRefresh?.finalRadius ?? targetBody?.lastRadiusRefresh?.radius) || null,
        minClampApplied: targetBody?.lastRadiusRefresh?.minClampApplied === true,
        maxClampApplied: targetBody?.lastRadiusRefresh?.maxClampApplied === true,
        clampApplied: targetBody?.lastRadiusRefresh?.clampApplied === true,
        clampReason: targetBody?.lastRadiusRefresh?.clampReason || null,
        usedCollisionRules: Boolean(rule),
        usedMassRadiusContract: Boolean(window.HC?.SpaceBodies?.massRadiusContract || targetBody?.massRadiusContractVersion),
      }, evidence, {
