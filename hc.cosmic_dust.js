...
(function () {
  ...
  function applySplitPolicy(World, spec) {
    ...
    else if (kind === "meteor_asteroid") { const meteor = s.meteor || (a?.type === "meteor" ? a : b); const asteroid = s.asteroid || (a?.type === "asteroid" ? a : b); const m = bodyMass(meteor); dust(m * rulePct(world, rule, "dustPct", "cosmicDustSplitMeteorAsteroidDustPct"), asteroid, meteor); evidence.absorbedMass = m * rulePct(world, rule, "absorbPct", "cosmicDustSplitMeteorAsteroidAbsorbPct"); addMass(asteroid, evidence.absorbedMass); resizeAsteroid(world, asteroid); if (meteor) meteor._dead = true; }
    else if (kind === "asteroid_asteroid") { const primary = s.primary || (bodyMass(a) >= bodyMass(b) ? a : b); const secondary = s.secondary || (primary === a ? b : a); const m = bodyMass(secondary); dust(m * rulePct(world, rule, "dustPct", "cosmicDustSplitAsteroidAsteroidDustPct"), primary, secondary); evidence.absorbedMass = m * rulePct(world, rule, "absorbPct", "cosmicDustSplitAsteroidAsteroidAbsorbPct"); addMass(primary, evidence.absorbedMass); resizeAsteroid(world, primary); if (secondary) secondary._dead = true; }
