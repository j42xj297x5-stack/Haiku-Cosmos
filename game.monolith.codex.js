    ...
    cards: [
      {
        ...
        windowMs: 5000,
        cooldownMs: 0,
        tags: ["ritual", "release", "single"],
        ui: { hint: "Uwolnij wolne meteory jednego koloru (nie wchodzą na orbity)." },
        // instance will provide: colorName
        effects: [{ op: "RITUAL_RELEASE_SINGLE_COLOR" }]
      },
      {
        ...
        windowMs: 5000,
        cooldownMs: 0,
        tags: ["ritual", "release", "dual"],
        ui: { hint: "Uwolnij wolne meteory dwóch kolorów naraz." },
        // instance will provide: colors[2]
        effects: [{ op: "RITUAL_RELEASE_DUAL_COLOR" }]
      }
    ]
  };

  // NOTE: For v1 rytuałów nie ładujemy presetów ani innych paczek.
  const PACK = normalizePack(FALLBACK_PACK);
...
const config = {
  ...
  // --- External offer helpers (used by Targets/Rytuały) ---
  function createCardInstanceById(cardId, extra = null) {
    const base = PACK.cards.find(x => x.id === cardId);
    if (!base) return null;
    const c = (structuredClone ? structuredClone(base) : JSON.parse(JSON.stringify(base)));
  ...
  function offerCardInstance(card) {
    if (!card) return false;
    if (!card._instanceId) card._instanceId = `${card.id}::${Math.random().toString(16).slice(2)}`;
    return enqueueOffer(card);
  }
  ...
  function applyEffects(effects) {
    ...
    for (const ef of effects) {
      // Custom ops for Rytuały (Targets v1)
      if (ef && (ef.op === "RITUAL_RELEASE_SINGLE_COLOR" || ef.op === "RITUAL_RELEASE_DUAL_COLOR")) {
        const w = state.world;
        if (!w) continue;
        if (!w.ritualRelease) w.ritualRelease = { active: false, colors: [], untilMs: 0 };
        const durMs = Number(ef.durationMs || 0) || (ef.op === "RITUAL_RELEASE_DUAL_COLOR" ? 120000 : 180000);
        if (ef.op === "RITUAL_RELEASE_SINGLE_COLOR") {
          const col = (state.activeOffer && state.activeOffer.card && state.activeOffer.card.colorName) ? state.activeOffer.card.colorName : null;
          if (!col) continue;
          w.ritualRelease.active = true;
          w.ritualRelease.colors = [col];
          w.ritualRelease.untilMs = nowMs() + durMs;
        } else {
          const cols = (state.activeOffer && state.activeOffer.card && state.activeOffer.card.colors) ? state.activeOffer.card.colors : null;
          if (!cols || cols.length < 2) continue;
          w.ritualRelease.active = true;
          w.ritualRelease.colors = [cols[0], cols[1]];
          w.ritualRelease.untilMs = nowMs() + durMs;
        }
        continue;
      }
  ...
  function update(dt, now) {
    ...
    if (state.activeOffer && t >= state.activeOffer.expiresAt) {
      ...
      if (c.collectOnExpire === false) {
        // decision cards can expire without being collected
      } else {
  ...
  function handlePointerDown,
    ...
    // external
    createCardInstanceById,
    offerCardInstance(mx, my, screenW, screenH) {
  ...
  Events.on("METEOR_SPAWNED", () => {
    // Rytuały (R1/R2) nie są losowane – karty pojawiają się wyłącznie z logiki rytuałów.
...
/* =========================================================
   RYTUAŁY v1 — zgodnie z targets_system.md
   - R1: Puszczanie (single color) -> karta po 3x harmonijnej kolizji tego samego koloru
   - Start stanu: 2x harmonijnej kolizji tego samego koloru
   - Przerwanie: harmonijna kolizja innego koloru w trakcie stanu
   - R2: Puszczanie Złączone (dual) -> po ukończeniu R1 A i R1 B bez przerwania ciągu (decyzja 5s)
   ========================================================= */
...
const Rituals = (() => {
  ...
  function breakChain(nowMs) {
    S.chainFirstColor = null;
    S.chainActive = false;
    S.chainExpiresAt = 0;
  }

  function isChainValid(nowMs) {
    return S.chainActive && nowMs < S.chainExpiresAt && !!S.chainFirstColor;
  }

  function startR1(color, nowMs) {
    S.active = true;
    S.activeColor = color;
    S.activeCount = 2; // bo start jest na 2x kolizji
    // (wizual/stan świata możemy dodać później)
  }

  function completeR1(color, nowMs) {
    // Offer karta R1 z przypiętym kolorem (3 min)
    const c = CardEngine.createCardInstanceById("RITUAL_RELEASE_SINGLE_COLOR", {
      title: `Puszczanie — ${color}`,
      colorName: color,
      windowMs: 5000
    });
    if (c) CardEngine.offerCardInstance(c);

    // chain logic: po pierwszym ukończeniu czekamy na drugi kolor
    if (!isChainValid(nowMs)) {
      S.chainFirstColor = color;
      S.chainActive = true;
      S.chainExpiresAt = nowMs + 30000; // 30s na domknięcie drugiego koloru (bez przerwania)
    } else {
      // jeśli chain już aktywny i ukończono drugi kolor:
      if (S.chainFirstColor && S.chainFirstColor !== color) {
        const d = CardEngine.createCardInstanceById("RITUAL_RELEASE_DUAL_COLOR", {
          title: `Puszczanie Złączone — ${S.chainFirstColor} + ${color}`,
          colors: [S.chainFirstColor, color],
          windowMs: 5000,
          collectOnExpire: false // brak reakcji = nie łączymy, więc karta decyzji znika
        });
        if (d) CardEngine.offerCardInstance(d);
      }
      breakChain(nowMs);
    }
  ...
  // METEOR_SAME_COLOR_COLLISION jest emitowany w resolveMeteorCollisionsSafe()
  Events.on("METEOR_SAME_COLOR_COLLISION", ({ color }) => {
    const nowMs = (World.nowMs ?? performance.now());

    // jeśli trwa R1 i trafiliśmy inny kolor -> przerwanie rytuału
    if (S.active && color !== S.activeColor) {
      resetActive();
      resetStreak();
      // COMBO / Rezonans Points ×2 (na razie prosto jako bonus score)
      try { addScore(2); } catch (e) {}
      breakChain(nowMs);
      return;
    }

    // streak tracking (dla startu na 2x)
    if (S.streakColor === color) S.streakCount++;
    else { S.streakColor = color; S.streakCount = 1; }

    if (!S.active) {
      if (S.streakCount >= 2) startR1(color, nowMs);
      return;
    }

    // aktywny i ten sam kolor
    if (color === S.activeColor) {
      S.activeCount++;
      if (S.activeCount >= 3) {
        completeR1(color, nowMs);
      }
    }
  });
...
// [ANCHOR:PLANETS]
  function captureMeteorsByPlanets(dt, nowMs) {
    ...
    for (let mi = meteors.length - 1; mi >= 0; mi--) {
      ...
      // Rytuał Puszczanie: wolne meteory objęte rytuałem nie mogą wchodzić na orbity
      if (World.ritualRelease && World.ritualRelease.active && (World.nowMs ?? nowMs) < World.ritualRelease.untilMs) {
        const cols = World.ritualRelease.colors || [];
        if (cols.indexOf(m.colorName) !== -1) continue;
      }
...
// [ANCHOR:ASTEROIDS]
  ...
  /* =========================================================
     [ANCHOR:CODEX_API]
     Public, stable hooks for Codex patches (NO gameplay changes)
     ========================================================= */
  window.HC = window.HC || {};
  window.HC.version = window.HC.version || "MONOLITH_SAFE_v4.1";
  window.HC.get = window.HC.get || function () {
    // Expose references for debugging / patches. Keep minimal to avoid accidental misuse.
    return {
      // Core runtime objects (add more if you need them in patches):
      ...
      // Card system (present in your file):
      CardEngine: (typeof CardEngine !== "undefined") ? CardEngine : undefined,
